%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% For the nonlinear least square algorithm, it has a fixed initial point
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%while tot_num_localization <= 1000
    ...
    while dr_move < 700
        ...
        dist = sqrt(sum(abs([l_DR(dr_move,1) l_DR(dr_move,2)] - l_TG).^2,2)) ;       % measured distance
...
xlabel('X Position');
ylabel('Y Position');
plot(l_DR(:,1), l_DR(:,2),'g-'); hold on;  % Drone Trajectory
plot(est_T(2:size(est_T),1), est_T(2:size(est_T),2), 'b-square'); hold on;   % Estimated Location of the target

...
while tot_num_localization <= 1000
    ...
    est_T = [0 0];             % estimated locatioin vector
    ...
    while dr_move < 720         % for boundary trajectory
        rt_cnt = 1;     % movement count in a loop
        if size(l_DR,1) == 1        % for the first loop, get two locations of drone
            while rt_cnt < 3
                if (dr_move >= 620) && (dr_move < 720)
                    l_x = l_x - 0;
                    l_y = l_y - 5;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 520) && (dr_move < 620)
                    l_x = l_x + 5;
                    l_y = l_y + 0;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 420) && (dr_move < 520)
                    l_x = l_x + 0;
                    l_y = l_y + 5;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 340) && (dr_move < 420)
                    l_x = l_x - 5;
                    l_y = l_y + 0;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 260) && (dr_move < 340)
                    l_x = l_x - 0;
                    l_y = l_y - 5;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 200) && (dr_move < 260)
                    l_x = l_x + 5;
                    l_y = l_y + 0;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 120) && (dr_move < 200)
                    l_x = l_x + 0;
                    l_y = l_y + 5;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 80) && (dr_move < 120)
                    l_x = l_x - 5;
                    l_y = l_y + 0;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 40) && (dr_move < 80)
                    l_x = l_x - 0;
                    l_y = l_y - 5;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move >= 20) && (dr_move < 40)
                    l_x = l_x + 5;
                    l_y = l_y + 0;
                    l_DR = [l_DR; l_x l_y];
                elseif (dr_move > 0) && (dr_move < 20)
                    l_x = l_x + 0;
                    l_y = l_y + 5;
                    l_DR = [l_DR; l_x l_y];
                end
        ...
        %% Distance Estimation
        dist_est = 10.^(-( r_sig_tot_pw_dB - G_t_dB - P_Tx_dB - 27.55 + 20*log10(fc) )/20) ; % estimate distance
        ...
        d_1 = dist_est(idx_DR(1)) ; % distance
        d_2 = dist_est(idx_DR(2)) ;
        d_3 = dist_est(idx_DR(3)) ;
...
% UAV Trajectory
figure(1);
plot(l_DR(:,1), l_DR(:,2), 'k-'); hold on
xlabel('X Position');
ylabel('Y Position');

% Estimated location & Target location
plot(est_T(2:size(est_T),1), est_T(2:size(est_T),2), "b:o")
hold on

plot(l_TG(1), l_TG(2), 'r*')
